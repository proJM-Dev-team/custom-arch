...
RUN install-packages-build glib2-devel lshw python-pipx
...
RUN runuser -u aur -- env -C /tmp_build/custom-arch/scripts/colour-icons git clone 'https://github.com/PapirusDevelopmentTeam/papirus-icon-theme.git' && \
    runuser -u aur -- env -C /tmp_build/custom-arch/scripts/colour-icons pipx install basic_colormath && \
    mkdir /usr/share/icons/papirus-gruvbox && \
    chown -R aur:aur /usr/share/icons/papirus-gruvbox && \
